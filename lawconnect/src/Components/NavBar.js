...
export default function NavBar()
{
  ...
  const str={backgroundColor: "rgb(0, 0, 0)"};
    ...
    <Link className="navbar-brand" to="/">LawConnect</Link>
    ...
    <div className={`${isNavCollapsed ? 'collapse' : ''} navbar-collapse`}  id="navbarSupportedContent">
      <ul className="navbar-nav me-auto mb-2 mb-lg-0">
        ...
        <li className="nav-item">
          <Link className={`nav-link ${location.pathname==="/myprofile"?"active":""}`} to="/myprofile">My-Profile</Link>
        </li>
        <li className="nav-item">
          <Link className={`nav-link ${location.pathname==="/connection"?"active":""}`} to="/connection">{user==="client"?"Lawyers":"My-clients"}</Link>
        </li>
        <li className="nav-item">
          <Link className={`nav-link ${location.pathname==="/appointments"?"active":""}`} to="/appointments">Appointments</Link>
        </li>
        {location.pathname==="/view"?<li className="nav-item">
          <Link className={`nav-link ${location.pathname==="/view"?"active":""}`} to="/view">{user==="client"?"Lawyer-Profile":"Client-Profile"}</Link>
        </li>:""}
        </>:""
        }
        </ul>
